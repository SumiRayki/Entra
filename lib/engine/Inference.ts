...
import { compressContextIfNeeded } from './ContextCompression'
...
async function chatInferenceStream() {
    ...
    if (
        mmkv.getBoolean(AppSettings.AutoCompressContext) &&
        !mmkv.getBoolean(AppSettings.BypassContextLength)
    ) {
        const compression = await compressContextIfNeeded({
            fields: fields,
            onAbortReady: (abort) => {
                useInference.getState().setAbort(async () => {
                    await abort()
                    await Chats.useChatState.getState().stopGenerating()
                })
            },
        })
        ...
        if (compression) {
            await Chats.useChatState
                .getState()
                .setContextSummary(compression.summary, compression.endOrder)
            fields.contextSummary = compression.summary
            fields.contextSummaryEndOrder = compression.endOrder
        }
    }
...
const titleGeneratorStream = async (chatId: number) => {
    ...
    fields.onEnd = () => {
        const title = parseReasoningText(output).content
...
async function obtainFields(): Promise<APIBuilderParams | void> {
    try {
        ...
        if (!apiConfig) {
            Logger.errorToast(`未找到配置 "${apiValues?.configName}"`)
            return
        }
        ...
        const contextLength = apiConfig.model.useModelContextLength
            ...
            : instructLength
        const length = Math.max(0, contextLength - (samplers.genamt as number))
        ...
        return {
            ...
            contextSummary: useCompressedContext
                ? (chatState.data?.context_summary ?? '')
                : '',
            contextSummaryEndOrder: useCompressedContext
                ? (chatState.data?.context_summary_end_order ?? -1)
                : -1,
        }
    } catch (e) {
        Logger.stackTrace(e)
        Logger.errorToast('请求构建失败: ' + e)
    }
}

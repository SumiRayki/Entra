...
export const characters = sqliteTable('characters', {
    ...
    chat_background_image: integer('chat_background_image', { mode: 'number' }),
    // Entra: Detailed attributes
    gender: text('gender').notNull().default(''),
    age: text('age').notNull().default(''),
    height: text('height').notNull().default(''),
    weight: text('weight').notNull().default(''),
    background_story: text('background_story').notNull().default(''),
    ...
    nsfw_description: text('nsfw_description').notNull().default(''),
    nsfw_cup_size: text('nsfw_cup_size').notNull().default(''),
    nsfw_hip: text('nsfw_hip').notNull().default(''),
    nsfw_sensitive_areas: text('nsfw_sensitive_areas').notNull().default(''),
    nsfw_orientation: text('nsfw_orientation').notNull().default(''),
})
...
export const chats = sqliteTable('chats', {
    ...
    context_summary_end_order: integer('context_summary_end_order', { mode: 'number' })
        .notNull()
        .default(-1),
})
...
export const instructs = sqliteTable('instructs', {
    ...
    // Entra: Jailbreak prompt system
    jailbreak_enabled: integer('jailbreak_enabled', { mode: 'boolean' }).notNull().default(false),
    jailbreak_prompt: text('jailbreak_prompt').notNull().default(''),
    jailbreak_position: text('jailbreak_position', {
        enum: ['before_system', 'after_system', 'after_history'],
    })
        .notNull()
...
export const lorebookEntries = sqliteTable('lorebook_entries', {
    ...
    // Entra: NSFW support - entries can be marked NSFW individually
    nsfw: integer('nsfw', { mode: 'boolean' }).notNull().default(false),
})
...
// ADVENTURES (Text Adventure / Interactive Fiction)

export const adventures = sqliteTable('adventures', {
    id: integer('id', { mode: 'number' }).primaryKey({ autoIncrement: true }),
    name: text('name').notNull().default('新冒险'),
    description: text('description').notNull().default(''),
    scenario: text('scenario').notNull().default(''),
    system_prompt: text('system_prompt').notNull().default(''),
    image_id: integer('image_id', { mode: 'number' }).$defaultFn(() => Date.now()),
    last_modified: integer('last_modified', { mode: 'number' })
        .$defaultFn(() => Date.now())
        .$onUpdateFn(() => Date.now()),
})

export const adventureCharacters = sqliteTable('adventure_characters', {
    id: integer('id', { mode: 'number' }).primaryKey({ autoIncrement: true }),
    adventure_id: integer('adventure_id', { mode: 'number' })
        .notNull()
        .references(() => adventures.id, { onDelete: 'cascade' }),
    character_id: integer('character_id', { mode: 'number' })
        .references(() => characters.id, { onDelete: 'set null' }),
    role: text('role', { enum: ['npc', 'supporting'] }).notNull().default('npc'),
    name: text('name').notNull().default(''),
    brief_description: text('brief_description').notNull().default(''),
})

export const adventureChats = sqliteTable('adventure_chats', {
    id: integer('id', { mode: 'number' }).primaryKey({ autoIncrement: true }),
    adventure_id: integer('adventure_id', { mode: 'number' })
        .notNull()
        .references(() => adventures.id, { onDelete: 'cascade' }),
    chat_id: integer('chat_id', { mode: 'number' })
        .notNull()
        .references(() => chats.id, { onDelete: 'cascade' }),
    active_protagonist_id: integer('active_protagonist_id', { mode: 'number' })
        .references(() => characters.id, { onDelete: 'set null' }),
})

export const adventuresRelations = relations(adventures, ({ many }) => ({
    characters: many(adventureCharacters),
    chats: many(adventureChats),
}))

export const adventureCharactersRelations = relations(adventureCharacters, ({ one }) => ({
    adventure: one(adventures, {
        fields: [adventureCharacters.adventure_id],
        references: [adventures.id],
    }),
    character: one(characters, {
        fields: [adventureCharacters.character_id],
        references: [characters.id],
    }),
}))

export const adventureChatsRelations = relations(adventureChats, ({ one }) => ({
    adventure: one(adventures, {
        fields: [adventureChats.adventure_id],
        references: [adventures.id],
    }),
    chat: one(chats, {
        fields: [adventureChats.chat_id],
        references: [chats.id],
    }),
}))
